...
void PRM::updateZone(const std::shared_ptr<octomap::OcTree>& tree, octomap_planner_utils::AABB zone, bool map_update)
{
  ...
  for (auto &node : nodes_){
    ...
    if (octomap_planner_utils::intersect(zone, node->position))
    {
      ...
      if (node->age > node_max_age_/(node->neighbors.size()+1) \
            || (map_update && !octomap_planner_utils::isFreeSpace(node->position, free_space_diameter_, tree)) || node->neighbors.size() == 0)
      {
        invalids++;
        node->valid = false;
      }
  ...
  while ( i < sample_cnt)
  {
    octomap::point3d sample = octomap_planner_utils::getSampleFromAABB(zone);
    if (octomap_planner_utils::isFreeSpace(sample, 1.1*free_space_diameter_, tree))
    {
      addNode(sample);
    }
    i++;
...
void PRM::addNode(octomap::point3d position)
{
  ...
  if (!close_nodes.empty() && position.distance(close_nodes[0].position) < min_node_distance_)
  {
    return;
  }
  ...
  for (auto &neighbor : close_nodes)
  {
    ...
    else if (octomap_planner_utils::isFreeSpace(node->position + (nodes_[nidx]->position - node->position)*(0.5/dist), free_space_diameter_, tree_))
    {
...
std::vector<octomap::point3d> PRM::findPath(octomap::point3d start, octomap::point3d goal, octomath::Vector3 velocity)
{
  ...
  //   return path;
  // }
  if (!octomap_planner_utils::isFreeSpace(goal, free_space_diameter_/2.0, tree_))
  {
...
// combines findPath and a forward+reverse simplifyFreeSpacePath pass into a single call
std::vector<octomap::point3d> PRM::findSimplifiedPath(octomap::point3d start, octomap::point3d goal, octomath::Vector3 velocity)
{
  auto path = findPath(start, goal, velocity);
  if (path.size() < 2)
  {
    return path;
  }
...
std::vector<octomap::point3d> PRM::simplifyFreeSpacePath(std::vector<octomap::point3d> path)
{
  ...
  while (next != path.size()-1)
  {
    ...
    {
      temp_point += delta;
      if (!octomap_planner_utils::isFreeSpace(temp_point, free_space_diameter_, tree_))
      {
        obstacle = true;
        break;
      }
...
double PRM::distance(octomap::point3d start, octomap::point3d goal)
{
  ...
  if (path.size() == 0){
    return octomap_planner_utils::INVALID_DISTANCE;
  }
...
octomap_planner_utils::path_info_t PRM::extraDistance(octomap::point3d start, octomap::point3d goal)
{
  ...
  auto path = findPath(start, goal, octomath::Vector3(0.0,0.0,0.0));
  if (path.size() == 0){
    return octomap_planner_utils::path_info_t{.distance=octomap_planner_utils::INVALID_DISTANCE};
  }
